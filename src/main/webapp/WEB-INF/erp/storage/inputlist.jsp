...
.finance-table {
        width: 300px;
        text-align: center;
        bdelivery-collapse: collapse;
        background-color: #f5e1c5; /* 연한 베이지 배경 */
        bdelivery-radius: 12px; /* 테이블 모서리 둥글게 */
    ...
    .finance-table td {
        padding: 12px;
        bdelivery-bottom: 1px solid #ddd;
        font-weight: bold;
    }
...
<body id="page-top">
	...
	<div id="wrapper" class="container1">
		...
		<!-- End of Sidebar -->
				...
				<div class="container-fluid">
					...
					<button class="btn btn-success mb-3" onclick="location.href='inventoryList'">재고</button>
					<!-- Page Heading -->
					...
					<div class="card shadow mb-4">
						...
						$(document).ready(function () {
						    const URL = "/getdelListajax"; 
						    var vm = Vue.createApp({
						        name: "App",
						        data() {
						            return {
						                msg: "입출고 리스트(vue-axios)", 
						                delList: [], 
						                delivery: { p_name: '', cnt: '',inputtype:''},
						                editdelivery: {},  // 수정할 데이터를 저장할 객체
						                isLoading: false,
						                searchPname: '',
						                searchStartDate: '',
						                searchEndDate: ''
						        ...
						        computed: {
						            // 총 입고 금액 (비용)
						            totalExpense() {
						                return this.delList.reduce((sum, delivery) => {
						                    return delivery.inputtype === "입고" ? sum + (delivery.price * delivery.cnt) : sum;
						                }, 0);
						            },
						            // 총 출고 금액 (수익)
						            totalIncome() {
						                return this.delList.reduce((sum, delivery) => {
						                    return delivery.inputtype === "출고" ? sum + (delivery.price * delivery.cnt) : sum;
						                }, 0);
						            },
						            // 최종 합산 (출고 - 입고)
						            totalProfit() {
						        ...
						        methods: {
						            ...
						            openEditModal(delivery) {
						                console.log("🛠 수정할 주문:", delivery);
						                this.editdelivery = { ...delivery }; // 기존 데이터를 복사
						                $('#editdeliveryModal').modal('show'); // 모달 띄우기
						            },

						            // 주문 수정 요청
						            updatedelivery() {
						                console.log("🚀 주문 수정 요청:", this.editdelivery);
						                axios.post("/ajaxdelListupt", this.editdelivery)
						                    .then(response => {
						                        alert(response.data.msg); // 성공 메시지 출력
						                        $('#editdeliveryModal').modal('hide'); // 모달 닫기
						                        this.editdelivery = {}; // 데이터 초기화
						                        this.getdelListajax(); // 리스트 새로고침
						                    })
						                    .catch(error => {
						            ...
						            // 주문 등록 요청
						            insertdelivery() {
						                axios.get("/ajaxdelListins", { params: this.delivery })
						                    .then((response) => {
						                        alert(response.data.msg);
						                        this.delivery = { p_name: '', cnt: '',inputtype:''};
								...
								<div class="container">
									...
									<div class="form-inline mb-3">
										<div class="delivery-header">
											<h2 class="delivery-title">입출고관리</h2>
											&nbsp;&nbsp;&nbsp;&nbsp;
											<div class="search-container">
												...
												<button class="btn btn-primary" @click="searchdelList">검색</button>
											</div>
										</div>

									</div>
										<button class="btn btn-success mb-3" data-toggle="modal"
											data-target="#deliveryModal">입출고 등록</button>
									<div class="table-responsive">
										...
										<table class="self table" id="customTable">
											...
											<tbody v-if="delList.length > 0">
												<tr v-for="delivery in delList"
													@dblclick="openEditModal(delivery)" style="cursor: pointer;">
													<td>{{ delivery.ncnt }}</td>
													<td>DI-{{ delivery.orderno }}</td>
													<td>{{ delivery.inputtype }}</td>
													<td>{{ delivery.p_name }}</td>
													<td>{{ delivery.delivery_date }}</td>
													<td>{{ delivery.price.toLocaleString() }}원</td>
													<td>{{ delivery.cnt.toLocaleString() }}개</td>
													<td>
    {{ (delivery.inputtype === "입고" ? "-" : "+") + (delivery.cnt * delivery.price).toLocaleString() }}원
</td>
    ...
    <table class="finance-table">
        ...
        <tbody>
            ...
            <tr>
                <td><strong>최종합산</strong></td>
                <td><strong>{{ totalProfit.toLocaleString() }}원</strong></td>
            </tr>
        </tbody>
    </table>
</div>
											...
											<div class="modal-dialog modal-dialog-centered"
												...
												<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title">입출고 정보 수정</h5>
													...
													<div class="modal-body">
														<form>
															<div class="form-group">
																<label>입출고 코드</label> <input type="text"
																	class="form-control" v-model="editdelivery.orderno">
															</div>
															<div class="form-group">
																<label>제품명</label> <input type="text"
																	class="form-control" v-model="editdelivery.p_name">
															</div>
															<div class="form-group">
																<label>가격</label> <input type="number"
																	class="form-control" v-model="editdelivery.price" readonly>
															</div>
															<div class="form-group">
																<label>수량</label> <input type="number"
																	class="form-control" v-model="editdelivery.cnt">
															</div>
														</form>
													</div>
													...
													<div class="modal-footer">
														...
														<button type="button" class="btn btn-primary"
															@click="updatedelivery">수정</button>
													</div>
												</div>
											</div>
										</div>
										...
										<div class="modal fade" id="deliveryModal" tabindex="-1"
											role="dialog" aria-labelledby="deliveryModalTitle"
											aria-hidden="true">
											<div class="modal-dialog modal-dialog-centered"
												role="document">
												<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title" id="deliveryModalTitle">입출고 등록</h5>
														<button type="button" class="close" data-dismiss="modal"
															aria-label="Close">
														...
														<form id="deliveryForm">
															<div class="form-group">
																<label for="p_name">제품명</label>
																 <select  v-model="delivery.p_name" name="p_name" class="form-control">
															...
															<div class="form-group">
																<label for="inputtype">분류</label>
																 			        <select  v-model="delivery.inputtype" name="inputtype"
																 			         class="form-control">
															...
															<div class="form-group">
																<label for="price">수량</label> <input type="number"
																	class="form-control" id="cnt" v-model="delivery.cnt"
																	placeholder="수량 입력">
															</div>
													...
													<div class="modal-footer">
														<button type="button" class="btn btn-secondary"
															data-dismiss="modal">취소</button>
														<button type="button" class="btn btn-primary"
															@click="insertdelivery">등록</button>
													</div>
												</div>
											</div>
										</div>
