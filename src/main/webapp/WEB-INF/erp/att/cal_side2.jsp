...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
	    ...
	    var calendar = new FullCalendar.Calendar(calendarEl, {
	        ...
	        locale: 'ko',
	        editable: false,
	        dayMaxEvents: true, // 많은 일정이 있을 때 "+ more" 링크 생성

	        // selectAllow에서 `selectInfo.view` 대신 `calendar.view.type`을 사용
	        selectAllow: function(selectInfo) {

		 var day = new Date(selectInfo.startStr).getDay();
	            if (day === 0 || day === 6) { //  주말이면 모달 실행하지 않음
	            	return;
	            } 
	        	
	        	console.log("selectAllow 실행됨:", selectInfo);
	            console.log("현재 캘린더 뷰 타입:", calendar.view.type);
	        ...
	        select: function(selectInfo) {
	            ...
	            // 선택된 일정 정보를 selectedEvent에 저장
	            selectedEvent = selectInfo;

	            // Bootstrap 모달 직접 호출 방식으로 변경
	            $('#showModal2').click(); 
	            console.log("모달 실행됨");
	            $("#leaveBtn").show();
	            console.log("버튼 실행 여부");
	            $("#wfhBtn").show();
	            $("#EndBtn").show();
	            
	            
	           // $("#StartBtn, #leaveBtn, #wfhBtn").show();
	            $("#uptBtn, #delBtn, #EndBtn").hide();
	            $("#modalTitle").text("일정 등록");

	            addForm(selectInfo, 'I'); 

	            // 선택 해제 실행을 지연하여 모달이 먼저 뜨도록 설정
	            setTimeout(() => calendar.unselect(), 100);
	        },

	        eventClick: function(arg) {
	            console.log("클릭된 일정 정보:", arg.event);

	            $('#exampleModalCenter2').modal('show'); // 모달 직접 실행
				console.log("모달 실행됨 (일정 클릭)");

	            //$("#regBtn, #StartBtn, #leaveBtn, #wfhBtn").hide();
	            //$("#EndBtn").show();
...
<!--   
<button type="button" class="btn btn-danger" id="ptoBtn">시차 등록</button>
 <button type="button" class="btn btn-success" id="StartBtn">출근 찍기</button> 
   <button type="button" class="btn btn-primary" id="EndBtn">퇴근 찍기</button> 
